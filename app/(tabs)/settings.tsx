...
import type { PraiseStyle, TaskLevel, UserSettings } from '@/src/types/storage';
import {
...
export default function SettingsScreen() {
  ...
  const saveLevel = async (lv: TaskLevel) => {
    ...
    const next: UserSettings = {
      praiseStyle: current.praiseStyle,
      level: lv,
    };
  ...
  const savePraiseStyle = async (style: PraiseStyle) => {
    setPraiseStyle(style); // 先に反映
  
    const current = normalizeUserSettings(
      await readJson<unknown>(STORAGE_KEYS.settings, null)
    );
    // 保存上の level（2|3 含む）は維持。UI表示 level はゲート済み
    const next: UserSettings = {
      level: current.level,
      praiseStyle: style,
    };
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]}>
      <ThemedView style={[styles.container, { backgroundColor: theme.background }]}>
        ...
        {/* ===== 褒め方 ===== */}
        <View style={styles.section}>
          <ThemedText style={styles.sectionTitle}>褒め方</ThemedText>

          <Pressable
            onPress={() => savePraiseStyle('gal')}
            style={[styles.choiceBtn, praiseStyle === 'gal' && styles.choiceBtnActive]}
          >
          ...
          <Pressable
            onPress={() => savePraiseStyle('serious')}
            style={[styles.choiceBtn, praiseStyle === 'serious' && styles.choiceBtnActive]}
          >
            <ThemedText style={styles.choiceText}>
              {praiseStyle === 'serious' ? '● ' : '○ '}真面目
            </ThemedText>
          </Pressable>
